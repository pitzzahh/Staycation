...
const RoomsDetailsPage = ({
  ...
  // RTK Query hooks
  const { data: wishlistStatus, error: wishlistError } =
    useCheckWishlistStatusQuery(
      { userId: userIdentifier, havenId: room.id },
      { skip: !userIdentifier },
    );
  ...
  const { data: reviewsResponse, isLoading: isLoadingReviews } =
    useGetHavenReviewsQuery({ haven_id: room.id });
  const reviewsData = reviewsResponse?.success
    ? reviewsResponse
    : { reviews: [], total: 0, hasMore: false };
  ...
  useEffect(() => {
    ...
    if (
      bookingData.checkInDate &&
      bookingData.checkInDate !== localCheckInDate
    ) {
      updates.push(() => setLocalCheckInDate(bookingData.checkInDate));
    }
    if (
      bookingData.checkOutDate &&
      bookingData.checkOutDate !== localCheckOutDate
    ) {
      updates.push(() => setLocalCheckOutDate(bookingData.checkOutDate));
    }
    if (
      bookingData.guests &&
      JSON.stringify(bookingData.guests) !== JSON.stringify(localGuests)
    ) {
      updates.push(() => setLocalGuests(bookingData.guests));
    }
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        dateDropdown &&
        !dateDropdown.contains(target) &&
        guestDropdown &&
        !guestDropdown.contains(target) &&
  ...
  const allPhotoTourImages = Object.values(photoTour)
    .flat()
  ...
  const handleWishlistToggle = async () => {
    try {
      ...
      toast.error(
        apiError?.data?.error || "An error occurred. Please try again.",
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Heart
                    className={`w-4 h-4 flex-shrink-0 ${isInWishlist ? "fill-red-500 text-red-500" : ""}`}
                  />
              ...
              {[1, 2, 3, 4].map((index) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      images[imageIndex] && openLightbox(imageIndex)
                    }
                  >
                    {images[imageIndex] ? (
                      <>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-500 dark:text-gray-400">
                      ({reviewsData?.total || room.reviews} reviews)
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
                    ...
                    <span>
                      Staycation Haven PH, Quezon City
                      {room.tower && `, ${room.tower}`}
                    </span>
                  </div>
                </div>
              </div>
                ...
                {activeTab === "overview" && (
                  <div className="space-y-8">
                      ...
                      <div className="bg-gradient-to-r from-brand-primary/10 to-orange-100/10 dark:from-brand-primary/20 dark:to-orange-900/20 p-6 rounded-xl border border-brand-primary/20 dark:border-brand-primary/30">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-semibold text-gray-900 dark:text-white">
                              {Math.ceil(
                                (new Date(localCheckOutDate).getTime() -
                                  new Date(localCheckInDate).getTime()) /
                                  (1000 * 60 * 60 * 24),
                              )}{" "}
                              night
                              {Math.ceil(
                    ...
                    <div>
                      ...
                      <p
                        className={`text-gray-600 dark:text-gray-300 leading-relaxed ${!showFullDescription && "line-clamp-4"}`}
                      >
                        {room.fullDescription || room.description}
                      </p>
                      {(room.fullDescription || room.description).length >
                        200 && (
                        <button
                          onClick={() =>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-3 text-sm text-gray-600 dark:text-gray-300">
                      Staycation Haven provides premium accommodations in Quezon
                      City. We ensure every guest has a comfortable and
                      memorable stay with 24/7 support.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto pb-4 scrollbar-thin">
                <div className="flex gap-4" style={{ minWidth: "max-content" }}>
                  {recommendedRooms.slice(0, 5).map((recRoom) => (
                    <div
                      key={recRoom.id}
                      className="w-[200px] sm:w-[220px] lg:w-[240px] flex-shrink-0"
                    >
                      <RoomCard room={recRoom} mode="browse" compact={false} />
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="lg:hidden fixed top-20 left-0 right-0 z-50 px-4 py-2">
        {/* Date Dropdown */}
        <div
          id="mobile-date-dropdown"
          className="hidden bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-4 mb-2"
        >
        ...
        {/* Guest Dropdown */}
        <div
          id="mobile-guest-dropdown"
          className="hidden bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-4"
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              Number of Guests
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                const guestDropdown = document.getElementById(
                  "mobile-guest-dropdown",
                );
                guestDropdown?.classList.add("hidden");
