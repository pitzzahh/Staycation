...
const DashboardPage = ({
  ...
  const calculateKPIs = () => {
    ...
    const occupancyRate =
      totalDays > 0 ? Math.round((activeBookings / totalDays) * 100) : 0;
    ...
    // Role Distribution
    const roleStats = employees.reduce(
      (acc, emp) => {
    ...
    const registrationStats = users.reduce(
      (acc, user) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {recentBookings.length > 0 ? (
              recentBookings.map((booking, index) => (
                ...
                >
                  ...
                  <span
                    className={`text-xs font-semibold px-2 py-1 rounded-full ${booking.color} bg-opacity-10`}
                  >
                    {booking.status}
                  </span>
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <button
              className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg p-4 text-center transition-all shadow-lg hover:shadow-xl transform hover:scale-105"
            ...
            <button
              className="bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 text-white rounded-lg p-4 text-center transition-all shadow-lg hover:shadow-xl transform hover:scale-105"
              onClick={(e) => e.preventDefault()}
            >
