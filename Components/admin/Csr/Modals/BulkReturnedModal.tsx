...
import { Loader2, CheckCircle, X, User, AlertCircle, DollarSign } from "lucide-react";
import { DepositRecord } from "@/app/admin/csr/actions";

interface BulkReturnedModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export default function BulkReturnedModal({
  isOpen,
  onClose,
  onConfirm,
  selectedDeposits,
  ...
  const handleConfirm = () => {
    ...
    sessionStorage.setItem('bulkReturnedNotes', JSON.stringify(notes));
    onConfirm();
  };
  ...
  const totalRefundAmount = selectedDeposits.reduce((total, id) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">
                Bulk Return Deposits
              </h2>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Process full refunds for {selectedDeposits.length} deposit{selectedDeposits.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  <strong>Important:</strong> This action will process full refunds totaling ₱{totalRefundAmount.toLocaleString()} to {selectedDeposits.length} guest{selectedDeposits.length !== 1 ? 's' : ''}. 
                  ...
                  <div key={id} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 resize-none"
                        rows={2}
                      />
                    </div>
                  </div>
        ...
        <div className="flex justify-end gap-2 p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
          ...
          >
            {isLoading ? (
              ...
              <>Return {selectedDeposits.length} Deposit{selectedDeposits.length !== 1 ? 's' : ''}</>
            )}
          </button>
        </div>
      </div>
