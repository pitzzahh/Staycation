...
interface BulkDeliverableActionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
const actionConfig = {
  preparing: {
    title: 'Mark All Items as Preparing',
    description: 'All items in this booking will be marked as currently being prepared for delivery.',
    icon: Clock,
    color: 'indigo',
  ...
  delivered: {
    ...
    description: 'All items in this booking will be marked as delivered and inventory stock will be updated.',
    icon: CheckCircle,
    color: 'green',
  ...
  cancelled: {
    ...
    description: 'All items in this booking will be cancelled and will not be delivered.',
    icon: XCircle,
    color: 'red',
  ...
  refunded: {
    title: 'Refund All Items',
    description: 'All items in this booking will be marked as refunded.',
    icon: AlertCircle,
    color: 'orange',
...
export default function BulkDeliverableActionModal({
  isOpen,
  onClose,
  onConfirm,
  action,
  ...
  items,
  isLoading = false
}: BulkDeliverableActionModalProps) {
    ...
    <>
      ...
      >
        ...
        <div className="p-6 space-y-6 max-h-[calc(90vh-200px)] overflow-y-auto">
          ...
          <div className={`${config.bgColor} ${config.borderColor} border rounded-lg p-4`}>
            <div className="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
              ...
              Affected Items ({affectedItems.length} of {items.length})
            </div>
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {affectedItems.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">
                  No items will be affected by this action
                </p>
              ) : (
                affectedItems.map((item) => (
                  <div key={item.id} className="flex justify-between items-center bg-white dark:bg-gray-800 rounded p-3">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{item.name}</p>
                      {item.notes && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Note: {item.notes}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <p className="text-xs text-gray-500 dark:text-gray-400">Qty: {item.quantity}</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{item.formatted_total}</p>
                      </div>
          ...
          {action === 'delivered' && affectedItems.length > 0 && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                    This action will automatically update inventory stock for all affected items:
                  </p>
                  <ul className="mt-2 space-y-1">
                    {affectedItems.map(item => (
                      <li key={item.id} className="text-sm text-amber-700 dark:text-amber-300">
                        • Subtract {item.quantity} unit(s) from "{item.name}"
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                    This action will update {affectedItems.length} item(s) and cannot be undone. Please confirm you want to proceed.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-6 flex justify-end gap-3 bg-gray-50 dark:bg-gray-800">
          ...
          <button
            onClick={onConfirm}
            disabled={isLoading || affectedItems.length === 0}
            ...
            {isLoading ? (
              ...
              <>
                ...
                {action === 'preparing' ? 'Mark All as Preparing' :
                 ...
                 'Refund All Items'}
                {affectedItems.length > 0 && ` (${affectedItems.length})`}
              </>
            )}
          </button>
        </div>
