        ...
        <div className="p-6 space-y-6 max-h-[calc(90vh-200px)] overflow-y-auto">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`inline-block px-2 py-1 rounded-full text-xs font-bold capitalize ${getStatusColor(booking.status)}`}>
                  {booking.status}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Room/Haven:</span>
                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">
                  {booking.room_name || 'N/A'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Total Guests:</span>
                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">
                  {totalGuests} (A:{booking.adults} C:{booking.children} I:{booking.infants})
                </span>
              </div>
              <div className="flex items-center gap-2">
              ...
              {booking.valid_id_url && (
                <div className="flex items-center gap-2">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-sm text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300 underline"
                  >
                    <ExternalLink className="w-3 h-3" />
                    View ID
                  </a>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-3">
            ...
            <div className="space-y-3">
              {/* Main Booking Payment (Down Payment) */}
              <div className="border-b border-gray-200 dark:border-gray-600 pb-3">
                <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Down Payment</h4>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  {booking.payment_proof_url && (
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Payment Proof:</span>
                      <a
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Security Deposit</h4>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {booking.security_deposit_payment_method ? (
                      <div className="flex items-center gap-1">
                        {booking.security_deposit_payment_method.toLowerCase() === 'cash' && <Banknote className="w-4 h-4 text-green-600" />}
                        {booking.security_deposit_payment_method.toLowerCase() === 'gcash' && <CreditCard className="w-4 h-4 text-blue-600" />}
                        {booking.security_deposit_payment_method.toLowerCase() === 'bank transfer' && <CreditCard className="w-4 h-4 text-purple-600" />}
                        <span className="text-sm font-medium text-gray-900 dark:text-gray-100 capitalize">
                          {booking.security_deposit_payment_method}
                        </span>
                      </div>
                      ...
                      <span className="text-sm text-gray-500 dark:text-gray-400 italic">
                        {!booking.deposit_status || booking.deposit_status === 'pending' ? 'Pending payment' : 'Not recorded'}
                      </span>
                    )}
                  </div>
