...
const MessageButton = () => {
  ...
  // Ensure a persistent guest identifier in localStorage so messages can be associated with the guest
  const getOrCreateGuestId = () => {
    try {
      let id = localStorage.getItem("guestId") as string | null;
      if (!id) {
        // Prefer crypto.randomUUID when available but avoid `any` casts for lint/type-safety
        const globalCrypto = (
      ...
      console.error("Failed to access localStorage for guestId", err);
      return `guest_${Date.now()}_${Math.floor(Math.random() * 100000)}`;
    }
  };

  // When the user opens the employee selection, fetch CSR employees if we haven't already.
  useEffect(() => {
    if (showEmployeeSelection && csrEmployees.length === 0) {
      fetchCsrEmployees();
    }
  }, [showEmployeeSelection, csrEmployees.length]);
  ...
  const handleEmployeeSelect = async (employee: Employee) => {
    ...
    // Create a persistent conversation for the guest user
    try {
      const convId = await createConversationWithEmployee(employee);
      if (convId) {
  ...
  const createConversationWithEmployee = async (employee: Employee) => {
    try {
      ...
      if (conversationResponse.ok) {
        ...
        const convId = conversationData?.data?.id;
        setConversationId(convId || null);
        return convId || null;
      } else {
        const err = await conversationResponse
          .json()
          .catch(() => ({ error: "Failed to create conversation" }));
        toast.error(err?.error || "Failed to create conversation");
        return null;
      }
    } catch (error) {
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    // Optimistically show the message in UI
    ...
    if (!convId) {
      if (!selectedEmployee) {
        toast.error(
          "Please select a support representative to start chatting.",
        );
        return;
      }
      convId = await createConversationWithEmployee(selectedEmployee);
      if (!convId) {
        // createConversationWithEmployee already shows an error toast
        return;
      }
      setConversationId(convId);
    }
    ...
    // Optional: keep a simple automated acknowledgement in the inline chat
    setTimeout(() => {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/bookings/search?booking_id=${encodeURIComponent(bookingId)}`,
      );
      const data = await response.json();
            ...
            !showEmployeeSelection ? (
              ...
              <div className="flex flex-col h-full">
                ...
                <form onSubmit={handleChatSubmit} className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={isSendingBackend || isCreatingConversation}
                    className="px-4 py-2 bg-brand-primary hover:bg-brand-primaryDark text-white rounded-lg transition-colors flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSendingBackend ? (
                      <svg
                ...
                {isLoadingEmployees ? (
                  ...
                  <div className="text-center py-8">
                    <p className="text-gray-500 dark:text-gray-400">
                      No customer service representatives available at the
                      moment.
                    </p>
                  </div>
                )}
                ...
                {bookingData && (
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3 bg-gray-50 dark:bg-gray-900/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(bookingData.status)}`}
                      >
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-gray-900 dark:text-gray-100">
                          {bookingData.guest_first_name}{" "}
                          {bookingData.guest_last_name}
                        </span>
                      </div>
